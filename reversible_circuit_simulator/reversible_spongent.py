...
def lfsr_state(round: int, initial_state: str = INITIAL_LFSR_STATE, taps: list = LFSR_TAPS):
...
def sbox_layer(gate_mapping: Mapping):
    ...
    for i in range(0, B, SBOX_WIDTH):
        sbox_layer.append_gate(sbox_gate(list(range(i, i + SBOX_WIDTH))))
...
def pi_permutation_gate(gate_mapping: Mapping, register_mapping: Mapping):
    ...
    return pi_permutation.to_gate(list(range(B)), "pi_permutation"), gate_mapping, register_mapping

def absorb_phase_gate(message_size: int):
    excess_bits = message_size % R
    padding_size = R - excess_bits

    absorb_phase = ReversibleCircuit(B + message_size - R) # The first 8 bits of the message are fed directly into the first 8 bits of the state
    ...
    return absorb_phase.to_gate(list(range(B + message_size - R)), "absorb_phase"), gate_mapping, register_mapping

def squeeze_phase_gate(message_size: int, gate_mapping: int, register_mapping: int):
    circuit_width = B + message_size - R + N # 88 bits for the state + (message_size - 8) bits for the message + 88 bits for the output hash

    squeeze_phase = ReversibleCircuit(circuit_width) 

    for i in range (0, N, R):
        for j in range(R):
            squeeze_phase.cx(gate_mapping[j], circuit_width - R - i + j)
        
        if i < B - R: # Don't add a permutation gate at the end.
            pi_permutation, gate_mapping, register_mapping = pi_permutation_gate(gate_mapping, register_mapping)
            squeeze_phase.append_gate(pi_permutation)
    
    return squeeze_phase.to_gate(list(range(circuit_width)), "squeeze_phase"), gate_mapping, register_mapping
...
c = ReversibleCircuit(B + message_size - R + N)
...
input_bits = "0"*N + "".join(reversed(message_bytes[1:])) + "0"*(B - R) + message_bytes[0]
